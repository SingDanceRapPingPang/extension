...
//        }
    ...
    public Graph createLinkedGraph(List<Triple> triples, Graph g){
        ...
        while (it.hasNext()){
            ...
            if(objectOfPredicate.getIsEntity()){
                ...
                Iterator<Vertex> iterator = vertexCollection.iterator();
    ...
    public Graph createLinkedGraphWithValues(List<Triple> triples, Graph g){
        ...
        while (it.hasNext()){
            ...
            /**if 'object' represents attribute values rather than an entity instance**/
                ...
                while (iterator.hasNext()){
                    ...
                    if(isValueEquals(vertex, objectOfPredicate)){
                        /**record id of the vertex**/
                        end_point = vertex.getId();
                        break;
                    }
                ...
                if(start_point.length()!=0){
                    ...
                    /**Case II: Start point exists in the linked data graph. However, end point does not exist in the linked data graph**/
                        ...
                        v.setData("data_value");
                        v.setValue(attributeValue);
                        System.out.println(v.getValue());
                        /**Add this new vertex into graph**/
                        g.addVertex(v);

                        /**Insert an edge from 'start_point' to 'end_point'**/
                        g.addEdge(start_point,v.getId(),RandomStringUUID.createUUID(),predicate.getPredicate(),true);
                    }
                }
                ...
                /**Case II: start point does not exist in the linked data graph**/
                    ...
                    /**Case IV: Neither start point nor end point exist in the linked data graph**/
                        ...
                        /**Insert an edge from 'start_point' to 'end_point'**/
                        g.addEdge(vertex.getId(), v.getId(),RandomStringUUID.createUUID(),predicate.getPredicate(),true);
                    }
     ...
     * */
    public static Graph createYagoGraph (List<Triple> yagoFacts, Map<String, YagoRelationship> yagoRelationshipHashMap) {
...
//        Iterator<YagoFact> it = yagoFacts.iterator();
        Iterator<Triple> it = yagoFacts.iterator();
...
//            String object = yagoFact.getObject();
            ...
            String object = triple.getObjectOfPredicate().toString();
...
//        ArrayList<String> commonList = new ArrayList<String> (CollectionUtils.retainAll(identificationCode,codes));
        if (identificationCode.size() != codes.size()){
            hasSameCode = false;
        }else {
            for (int i = 0; i < identificationCode.size(); i++) {
                if (!Objects.equals(identificationCode.get(i), codes.get(i))) {
                    hasSameCode = false;
                    break;
                }
            }
...
//        if((commonList.size()==identificationCode.size())&&(commonList.size()==codes.size())&&(label.equals(name))){
        if (hasSameCode && label.equals(name)){
            isEqual = true;
        }else {
            isEqual = false;
        }
    ...
    public boolean isValueEquals(Vertex v, ObjectOfPredicate objectOfPredicate){
        ...
        return v.getValue().equals(objectOfPredicate.getValue());
    }
